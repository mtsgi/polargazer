目的: 楽曲データ `common_getdata` / スコアデータ `pdata_getdata` / 譜面メタデータ `meta.json` を読み込み、プロフィールと楽曲自己ベストを表示する

- 方針: 責務分離（types/composables/utils/components）を維持する
- UI方針: 既存の共通UIコンポーネントを優先利用する
...
ファイル確認、テスト実行などはIDE機能を活用(なるべく不要なコマンド実行は避ける)
